...
The last question is the one most often skipped. A cache improves the normal path but can make the miss path worse: if a hot key serves 10,000 QPS and the cached value expires, every request can become a database request unless something coordinates the refill.
...
Early in the TTL, refresh is unlikely. Near the end, one of the readers is likely to refresh before the key fully expires. This is not needed everywhere; it is worth the extra writes on hot keys where a synchronized expiry would be costly.
...
Vague headers around private content should be avoided. The cache bug that matters most is not stale data; it is one user seeing another user's data because the cache key ignored identity.
...
This reduces database write pressure but creates a loss window: if Redis loses the buffer before flush, those views are gone. That is acceptable for views and not acceptable for payments.
